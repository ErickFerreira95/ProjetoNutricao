...
public class CalculoTmbView extends JFrame {
                ...
                try {
                    if (!emptyFields()) {
                        if (comboBox.getSelectedIndex() == 0) {
                            ...
                            tmb = fatorAtividade * (88.362 + (13.397 * Double.parseDouble(txtPeso.getText().replace(",", ".")))
                            ...
                            if (validarIdadeAltura(txtIdade.getText()) == false) {
                                JOptionPane.showMessageDialog(null, "Idade no formato incorreto!"
                                        + "Ex: 30");
                            } else if (validarIdadeAltura(txtAltura.getText()) == false) {
                                JOptionPane.showMessageDialog(null, "Altura no formato incorreto!"
                                        + "Ex: 175");
                            } else if (validarPesoFator(txtPeso.getText()) == false) {
                                JOptionPane.showMessageDialog(null, "Peso no formato incorreto!"
                                        + "Ex: 70 ou 70,5");
                            } else if (validarPesoFator(txtFatorAtividade.getText()) == false) {
                                JOptionPane.showMessageDialog(null, "Fator atividade no formato incorreto!"
                                        + "Ex: 1,3");
                            } else {
                                double fatorAtividade = Double.parseDouble(txtFatorAtividade.getText().replace(",", "."));
                                tmb = fatorAtividade * (447.593 + (9.247 * Double.parseDouble(txtPeso.getText().replace(",", ".")))
    ...
    public boolean validarPesoFator(String pesoFator) {
        ...
        Pattern pattern = Pattern.compile(regex);
        Matcher matcher = pattern.matcher(pesoFator);
        return matcher.matches();
    }
}
